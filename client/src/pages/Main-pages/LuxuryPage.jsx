...
import useLuxury from '../../hooks/packages/useLuxury';

const LuxuryPage = () => {
    const { luxury, isLoading } = useLuxury()
    useEffect(() => {
    ...
    const generalPackage = luxury?.filter((pkg) => pkg.packageType === "luxury" && pkg.customizablePackage === false) || []
    const customizePackage = luxury?.filter((pkg) => pkg.packageType === "luxury" && pkg.customizablePackage === true) || []
