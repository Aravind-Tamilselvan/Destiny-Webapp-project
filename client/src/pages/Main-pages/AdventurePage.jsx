...
import useAdventure from '../../hooks/packages/useAdventure';

const AdventurePage = () => {
    const { adventure, isLoading } = useAdventure()
    useEffect(() => {
        window.scrollTo(0, 0);
    }, []);
    ...
    const generalPackage = adventure?.filter((pkg) => pkg.packageType === "adventure" && pkg.customizablePackage === false) || []
    const customizePackage = adventure?.filter((pkg) => pkg.packageType === "adventure" && pkg.customizablePackage === true) || []
